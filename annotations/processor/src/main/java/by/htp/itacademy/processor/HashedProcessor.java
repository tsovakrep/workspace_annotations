...
@SupportedAnnotationTypes(value = { HashedProcessor.ANNOTATION_TYPE })
@SupportedSourceVersion(SourceVersion.RELEASE_8)
@SupportedOptions({ HashedProcessor.ENABLE_OPTIONS_NAME })
public class HashedProcessor extends AbstractProcessor {
	public static final String ANNOTATION_TYPE = "by.htp.itacademy.annotation.Hashed";
	public static final String ENABLE_OPTIONS_NAME = "Hashed";
	private static final String SUPPORT_FIELD_TYPE = "String";
	private JavacProcessingEnvironment javacProcessingEnv;
	private TreeMaker maker;
	private final static DevLog log = new DevLog("C:/1/logHashed.txt");
	private boolean enable = true;

	@Override
	public void init(ProcessingEnvironment procEnv) {
		log.log("procEnv: " + procEnv);
		super.init(procEnv);
		...
		log.log("javacProcessingEnv: " + javacProcessingEnv);
		log.log("maker: " + maker);
	}
	...
	public boolean process(Set<? extends TypeElement> annotations, RoundEnvironment roundEnv) {
		...
		log.log("roundEnv: " + roundEnv);
		if (!enable) {
			javacProcessingEnv.getMessager().printMessage(Diagnostic.Kind.NOTE, "Annotation Hashed is disable");
			return false;
		}
		...
		for (TypeElement annotation : annotations) {
			// убеждаемся, что аннотация наша (не уверен, что сюда могут попасть не наши,
			// но...)
					...
					if (blockNode instanceof JCTree.JCVariableDecl) {
						...
						if (!SUPPORT_FIELD_TYPE.equals(var.vartype.toString())) {
							javacProcessingEnv.getMessager().printMessage(Diagnostic.Kind.ERROR,
									String.format("Unsupported field type \"%s\" for hashed. Supported only String.",
											var.vartype.toString()),
									field);
							continue;
						}
						JCTree.JCExpression initializer = var.getInitializer();
						// Проверка отсечёт поля с инициализацией в конструкторе, а так же конструкции
						// вида:
						// "" + 1
						...
						if ((initializer != null) && (var.getInitializer() instanceof JCTree.JCLiteral)) {
							...
							log.log("before lit: " + lit);
							// получаем строку
							String value = lit.getValue().toString();
							log.log("before value: " + value);
							...
							try {
								...
								var.init = lit;
								log.log("after value: " + value);
								log.log("after lit: " + lit);
							} catch (NoSuchAlgorithmException e) {
								javacProcessingEnv.getMessager().printMessage(Diagnostic.Kind.ERROR,
										String.format("Unsupported digest method %s", hashed.method()), field);
							} catch (UnsupportedEncodingException e) {
								javacProcessingEnv.getMessager().printMessage(Diagnostic.Kind.ERROR,
										"Bad environment. Required to support UTF-8", field);
							}
						} else {
							javacProcessingEnv.getMessager().printMessage(Diagnostic.Kind.ERROR,
									"Bad annotation: supported only literal string. Example: \"Good string\" ", field);
						}

					} else {
						javacProcessingEnv.getMessager().printMessage(Diagnostic.Kind.ERROR,
								"Bad annotate. Supported only class variable field.", field);
					}
				}
