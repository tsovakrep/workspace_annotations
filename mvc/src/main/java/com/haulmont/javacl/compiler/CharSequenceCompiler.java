...
public class CharSequenceCompiler<T> {
	...
	public synchronized Map<String, Class<T>> compile(final Map<String, CharSequence> classes,
		...
		final JavaCompiler.CompilationTask task = compiler.getTask(null, javaFileManager, diagnostics, options, null,
				sources);
		...
		if (result == null || !result) {
			...
			throw new CharSequenceCompilerException("Compilation failed. Causes: " + cause, classes.keySet(),
					diagnostics);
		}
		try {
	 ...
	 * compiled by
	 * {@link #compile(String, CharSequence, javax.tools.DiagnosticCollector, Class...)
